<template>
  <div
    @click.stop="emit('close')"
    class="fixed fscreen inset-0 z-60 bg-black/80 text-white px-4 flex flex-col items-center pt-32 gap-6"
  >
    <!-- Icon -->
    <div class="flex justify-center">
      <img
        draggable="false"
        class="w-32 h-32 aspect-square rounded-xl object-cover"
        :src="getImageUrl(app.icon)"
      />
    </div>
    <!-- Icon -->
    <div
      class="flex items-center justify-center gap-2 overflow-x-auto scrollbar-hide"
    >
      <h1 class="badge badge-secondary">{{ app.title }}</h1>
      <h1 class="badge">{{ app.name }}</h1>
      <h1 class="badge">{{ app.theme }}</h1>
    </div>
    <!-- Buttons -->
    <div
      class="p-4 md:max-w-md max-w-xs w-full flex flex-col gap-3 border rounded-lg bg-white/20"
    >
      <button @click="openApp()" class="btn btn-sm">Open</button>
      <button class="btn btn-sm btn-info" @click="openApp(true)">
        Open as sudo
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="20"
          height="20"
          viewBox="0 0 24 24"
        >
          <path
            fill="currentColor"
            d="m16 7.58l-5.5-2.4L5 7.58v3.6c0 3.5 2.33 6.74 5.5 7.74c.25-.08.49-.2.73-.3c-.15-.51-.23-1.06-.23-1.62c0-2.97 2.16-5.43 5-5.91z"
            opacity="0.3"
          />
          <path
            fill="currentColor"
            d="M17 13c-2.21 0-4 1.79-4 4s1.79 4 4 4s4-1.79 4-4s-1.79-4-4-4m0 1.38c.62 0 1.12.51 1.12 1.12s-.51 1.12-1.12 1.12s-1.12-.51-1.12-1.12s.5-1.12 1.12-1.12m0 5.37c-.93 0-1.74-.46-2.24-1.17c.05-.72 1.51-1.08 2.24-1.08s2.19.36 2.24 1.08c-.5.71-1.31 1.17-2.24 1.17"
            opacity="0.3"
          />
          <circle cx="17" cy="15.5" r="1.12" fill="currentColor" />
          <path
            fill="currentColor"
            d="M18 11.09V6.27L10.5 3L3 6.27v4.91c0 4.54 3.2 8.79 7.5 9.82c.55-.13 1.08-.32 1.6-.55A5.97 5.97 0 0 0 17 23c3.31 0 6-2.69 6-6c0-2.97-2.16-5.43-5-5.91M11 17c0 .56.08 1.11.23 1.62c-.24.11-.48.22-.73.3c-3.17-1-5.5-4.24-5.5-7.74v-3.6l5.5-2.4l5.5 2.4v3.51c-2.84.48-5 2.94-5 5.91m6 4c-2.21 0-4-1.79-4-4s1.79-4 4-4s4 1.79 4 4s-1.79 4-4 4"
          />
          <path
            fill="currentColor"
            d="M17 17.5c-.73 0-2.19.36-2.24 1.08c.5.71 1.32 1.17 2.24 1.17s1.74-.46 2.24-1.17c-.05-.72-1.51-1.08-2.24-1.08"
          />
        </svg>
      </button>
    </div>
    <button
      @click.stop="showAlert = true"
      class="btn btn-sm btn-error max-w-xs md:max-w-sm w-full"
    >
      Uninstall
    </button>

    <Transition name="fade">
      <Alert
        v-if="showAlert"
        :message="`Do you want to uninstall '${app.title}' app?`"
        @onResponse="onResponse"
      />
    </Transition>
  </div>
</template>

<script setup>
  import { ref } from "vue";
  import { getImageUrl } from "@/kikx/config";

  import Alert from "@/components/ui/Alert.vue";

  const showAlert = ref(false);

  const props = defineProps(["app", "uninstallApp"]);
  const emit = defineEmits(["openApp", "close"]);

  function onResponse(success) {
    showAlert.value = false;
    if (success) {
      props.uninstallApp(props.app.name);
    }
  }

  function openApp(sudo = false) {
    emit("openApp", props.app.name, { sudo });
  }
</script>
